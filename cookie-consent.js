...
const config = {
    ...
    behavior: {
        ...
        darkModeToggle: true // Show dark mode toggle in settings
    },
...
const darkColorScheme = {
    ...
    textLight: '#b0b0b0',    // Lighter text for dark mode
    background: '#1a1a1a',   // Dark background
    toggleActive: '#2ecc71', // Same as primary
    toggleInactive: '#555555',// Dark gray for inactive
    darkMode: true           // Dark mode flag
};
...
const translations = {
    ...
    // ... (keep all other language translations the same, adding darkMode translation for each)
};

// ... (keep all other existing code the same until the injectConsentHTML function)
...
function injectConsentHTML(detectedCookies, language = 'en') {
    ...
    const darkModePref = getCookie('dark_mode_pref');
    const currentTheme = config.uiTheme === 'classic' ? 
        (darkModePref === 'true' ? classicDarkColorScheme : classicColorScheme) :
        (darkModePref === 'true' ? darkColorScheme : colorScheme);
    ...
    <div class="dark-mode-toggle">
        <label class="toggle-switch">
            <input type="checkbox" id="darkModeToggle" ${currentTheme.darkMode ? 'checked' : ''}>
            <span class="toggle-slider"></span>
    ...
    .adjust-btn {
        background-color: ${currentTheme.darkMode ? '#2d2d2d' : '#f8f9fa'};
        color: ${currentTheme.darkMode ? '#f5f5f5' : '#333'};
        border: 1px solid ${currentTheme.darkMode ? '#444' : '#e0e0e0'};
    }

    .adjust-btn:hover {
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#f0f2f5'};
        transform: translateY(-1px);
        box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08);
    }

    .reject-btn {
        background-color: ${currentTheme.darkMode ? '#1a1a1a' : '#fff'};
        color: ${currentTheme.danger};
        border: 1px solid ${currentTheme.danger};
    }

    .reject-btn:hover {
        background-color: ${currentTheme.darkMode ? '#2a1a1a' : '#ffeeed'};
        transform: translateY(-1px);
    ...
    .dark-mode-toggle {
        margin-bottom: 25px;
        padding: 15px;
        background-color: ${currentTheme.darkMode ? '#2d2d2d' : '#f5f5f5'};
        border-radius: 8px;
        display: flex;
        align-items: center;
    ...
    .language-selector select:hover {
        ...
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#fff'};
    }
    ...
    .cookie-settings-header {
        padding: 20px 30px;
        border-bottom: 1px solid ${currentTheme.darkMode ? '#444' : '#ecf0f1'};
        display: flex;
        justify-content: space-between;
        align-items: center;
        background-color: ${currentTheme.darkMode ? '#2d2d2d' : '#f8f9fa'};
    ...
    .cookie-settings-body {
        padding: 25px 30px;
        background-color: ${currentTheme.darkMode ? '#1a1a1a' : '#fefefe'};
        overflow-y: auto;
        flex: 1;
    }
    ...
    .cookie-details-header:hover {
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#eeeeee'};
    }

    .cookie-details-content {
        padding: 18px;
        background-color: ${currentTheme.darkMode ? '#2d2d2d' : '#fafafa'};
        border-top: 1px solid ${currentTheme.darkMode ? '#444' : '#e0e0e0'};
        display: none;
    ...
    .cookie-details-table th {
        text-align: left;
        padding: 10px 12px;
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#f0f0f0'};
        border-bottom: 2px solid ${currentTheme.darkMode ? '#444' : '#e0e0e0'};
        color: ${currentTheme.textDark};
    }

    .cookie-details-table td {
        padding: 10px 12px;
        border-bottom: 1px solid ${currentTheme.darkMode ? '#444' : '#e0e0e0'};
        color: ${currentTheme.textLight};
    }
    ...
    .cookie-details-table tr:hover {
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#f5f5f5'};
    }
    ...
    .cookie-settings-button {
        ...
        opacity: 0;
        transform: translateY(20px);
        border: 2px solid ${currentTheme.darkMode ? '#444' : 'white'};
    }
    ...
    .password-prompt input {
        ...
        margin-bottom: 15px;
        font-size: 14px;
        background-color: ${currentTheme.darkMode ? '#2d2d2d' : '#fff'};
        color: ${currentTheme.textDark};
    }
    ...
    .stat-bar {
        height: 20px;
        background-color: ${currentTheme.darkMode ? '#3d3d3d' : '#ecf0f1'};
        border-radius: 10px;
        overflow: hidden;
        display: flex;
    }
    ...
    .cookie-settings-footer {
        ...
        gap: 15px;
        border-top: 1px solid ${currentTheme.darkMode ? '#444' : '#ecf0f1'};
    }
...
function toggleDarkMode(enable) {
    ...
    setCookie('dark_mode_pref', enable.toString(), 365);
    ...
    // Reload the banner with new theme
    ...
    document.getElementById('cookieFloatingButton')?.remove();
    document.getElementById('cookieAdminButton')?.remove();
    document.getElementById('cookieAnalyticsModal')?.remove();
    
    // Re-inject with new theme
...
function generateCookieTable(cookies) {
    const currentTheme = getCookie('dark_mode_pref') === 'true' ? 
