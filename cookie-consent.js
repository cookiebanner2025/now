...
const translations = {
    ...
    // ... (keep all other language translations as in original code)
};
...
function showAnalyticsDashboard() {
    ...
    const lang = document.getElementById('cookieLanguageSelect')?.value || 'en';
    if (config.analytics.passwordProtect && !isDashboardAuthenticated) {
        document.querySelector('.cookie-analytics-body').innerHTML = generatePasswordPrompt(lang);
    } else {
        document.querySelector('.cookie-analytics-body').innerHTML = generateAnalyticsDashboard(lang);
    }
    modal.style.display = 'flex';
...
function updateConsentMode(consentData) {
    ...
    let gcsSignal = consentData.status === 'accepted' ? 'G111' : 
                    consentData.status === 'custom' ? 'G101' : 'G100';
...
function acceptAllCookies() {
    ...
    if (config.analytics.enabled) updateConsentStats('accepted', consentData);
}
...
function rejectAllCookies() {
    ...
    if (config.analytics.enabled) updateConsentStats('rejected', consentData);
}
...
function saveCustomSettings() {
    ...
    if (config.analytics.enabled) updateConsentStats('custom', consentData);
}
...
function loadAnalyticsCookies() {
    // Load Google Analytics if present in dataLayer
    if (window.dataLayer) {
        window.dataLayer.push({
        ...
        // Check for existing Google Analytics script and load if not present
...
function loadAdvertisingCookies() {
    // Load advertising cookies (e.g., Google Ads, Facebook Pixel)
    if (window.dataLayer) {
        window.dataLayer.push({
...
function loadPerformanceCookies() {
    // Load performance cookies (e.g., for speed optimization, CDN)
    if (window.dataLayer) {
        window.dataLayer.push({
        ...
        // Example: Load a performance monitoring script
...
// Update cookie tables in the UI
...
/**
 * Log consent actions for debugging and audit purposes
 */
...
/**
 * Refresh consent UI based on current consent state
 */
...
function checkConsentExpiration() {
    ...
    if (consentData) {
        const parsedConsent = JSON.parse(consentData);
        const consentAge = (new Date().getTime() - parsedConsent.timestamp) / (1000 * 60 * 60 * 24); // Age in days
        
        // If consent is older than 365 days, prompt for renewal
        if (consentAge > 365) {
            setCookie('cookie_consent', '', -1); // Expire the cookie
            showCookieBanner();
...
function initializePeriodicTasks() {
    // Check consent expiration every 24 hours
    setInterval(checkConsentExpiration, 24 * 60 * 60 * 1000);
    
    // Clean up analytics data every 24 hours
    if (config.analytics.enabled) {
        setInterval(cleanupAnalyticsData, 24 * 60 * 60 * 1000);
        cleanupAnalyticsData(); // Initial cleanup
    }
}
...
// Log initial load
logConsentAction('script_loaded', { version: '4.0' });
/**
 * Handle consent withdrawal (GDPR requirement)
 */
function withdrawConsent() {
    ...
    // Update consent mode to default (denied)
    gtag('consent', 'default', {
        'ad_storage': 'denied',
        'analytics_storage': 'denied',
        'ad_user_data': 'denied',
    ...
    // Reset analytics authentication if applicable
...
/**
 * Add withdraw consent button to settings modal
 */
...
/**
 * Error handling wrapper for critical functions
 */
...
document.addEventListener('DOMContentLoaded', () => {
    // Add withdraw consent option to settings
    addWithdrawConsentOption();
    
    // Re-run initialization with error handling
    safeInitialize();
    
    // Refresh UI based on current consent state
    refreshConsentUI();
...
// Export functions for potential external use (e.g., via a CMS)
...
/**
 * Check for script version updates
 */
function checkScriptVersion() {
    ...
    if (storedVersion !== currentVersion) {
        // Version has changed, clear old data and re-initialize
...
/**
 * Expose debug information for developers
 */
function getDebugInfo() {
    return {
        ...
        history: consentHistory.slice(-10), // Last 10 entries
...
// Add debug method to CookieConsent object
window.CookieConsent.debug = getDebugInfo;

/**
 * Final error boundary to prevent script from breaking the page
 */
try {
    // Check script version on load
    checkScriptVersion();
